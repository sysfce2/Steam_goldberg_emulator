...
void append_renderer_info()
{
    ...
    // Table + probing live in dll/known_tools.h so this file and
    // steam_client_interface_getter.cpp cannot drift apart.
    std::vector<DetectedTool> detected_tools;
    known_tools::collect(detected_tools);
    for (const auto &t : detected_tools)
        PRINT_DEBUG("detected tool [%s]: %s", t.type.c_str(), t.label.c_str());
...
void append_renderer_info()
{
    ...
    // Table + scan live in dll/known_tools.h so this file and
    // steam_client_interface_getter.cpp cannot drift apart.
    std::vector<DetectedTool> detected_tools;
    known_tools::collect(detected_tools);
    for (const auto &t : detected_tools)
